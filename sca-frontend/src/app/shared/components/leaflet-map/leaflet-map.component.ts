...
import { BRAZIL_GEOJSON } from './brazil-geojson';
...
export class LeafletMapComponent implements OnInit {
  ...
  @Input() layerBounds: any;
  @Input() ufOrCity: string;
  leafletMap: any;
  geoJSON: any;
  prevLayerClicked = null;
  complexMarker: any;
  @Input() complexMarkerLatLng: any;
  @Output() emitLatLng = new EventEmitter();
  ...
  ngOnInit(): void {
    ...
    this.geoJSON = L.geoJSON(this.geojsonLayer, {
      style: this.style,
      onEachFeature: this.ufOrCity === 'uf' ? this.onEachFeature : null,
    });
    // }).addTo(this.leafletMap);
    ...
    this.leafletMap.fitBounds(this.layerBounds);

    if (this.ufOrCity === 'city') {
      if (this.complexMarkerLatLng) {
        this.complexMarker = L.marker(this.complexMarkerLatLng).addTo(this.leafletMap);
      }

      this.leafletMap.on('click', (e) => {
        const lat = e.latlng.lat;
        const lng = e.latlng.lng;
        if (this.complexMarker) {
          this.leafletMap.removeLayer(this.complexMarker);
        }
        this.complexMarker = L.marker([lat, lng]).addTo(this.leafletMap);
        // console.log([lat, lng]);
        this.emitLatLng.emit([lat, lng]);
      });
    }
