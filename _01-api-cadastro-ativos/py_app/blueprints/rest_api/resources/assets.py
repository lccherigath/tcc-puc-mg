...
from py_app.models.mod1_ativos import Ativo
...
def list_equipments():
    try:
        ids_equipments = [5, 8, 9, 10, 15]
        equipments = Ativo.query.filter(~Ativo.categoria_id.in_(ids_equipments)).all()
        return_data = {
            'count': len(equipments),
            'next': None,
            'previous': None,
            'results': [equipment.to_dict() for equipment in equipments]
        }
        return return_data, 200
    except Exception as e:
        return error_handler.internal_error(e)
